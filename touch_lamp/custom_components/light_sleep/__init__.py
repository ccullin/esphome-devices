...
PreventLightSleepAction = light_sleep_ns.class_(
    "PreventLightSleepAction",
    automation.Action,
    cg.Parented.template(LightSleepComponent),
)
AllowLightSleepAction = light_sleep_ns.class_(
    "AllowLightSleepAction",
    automation.Action,
    cg.Parented.template(LightSleepComponent),
)
...
WakeupCauseToRunDuration = light_sleep_ns.struct("WakeupCauseToRunDuration")
...
DEEP_SLEEP_ACTION_SCHEMA = cv.Schema(
    {
        cv.GenerateID(): cv.use_id(LightSleepComponent),
    }
)
...
@automation.register_action(
    "light_sleep.enter", EnterLightSleepAction, DEEP_SLEEP_ENTER_SCHEMA
)
...
async def light_sleep_action_to_code(config, action_id, template_arg, args):
